...
class LinearRestrainFunction(AbstractRestrainFunction):
    ...
    def evaluate(self, d): # zamiast __call__
...
class FlatBottomRestrainFunction(AbstractRestrainFunction):
    ...
    def evaluate(self, d):  # Calculate the energy for distance d according to the formula
...
if __name__ == "__main__":
    ...
    for line in rst_data.split("\n"):
        ...
        j = int(tokens[1])
        if tokens[2] == "LINEAR":
            r = LinearRestrainFunction(i, j, float(tokens[3]), float(tokens[4]))
            rst.append(r)
             # call LinearRestrainFunction constructor here
        elif tokens[2] == "HARMONIC":
            r = HarmonicRestrainFunction(i, j, float(tokens[3]), float(tokens[4]))
            rst.append(r)
             # call HarmonicRestrainFunction constructor here
        elif tokens[2] == "LORENTZIAN":
            r = LorentzianRestrainFunction(i, j, float(tokens[3]), float(tokens[4]))
            rst.append(r)
             # call LorentzianRestrainFunction constructor here
        elif tokens[2] == "FLAT_BOTTOM":
            r = FlatBottomRestrainFunction(i, j, float(tokens[3]), float(tokens[4]), float(tokens[5]))
            rst.append(r)
              # call FlatBottomRestrainFunction constructor here
        else:
            print("UNKNOWN RESTRAINT TYPE:", line)
